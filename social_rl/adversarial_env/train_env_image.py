...
class MyRecorder:
  def __init__(self, path, fps=0):
    self.path = path
    self.fps = fps
    self.cur_frame_number = 0
  
  def add_frame(self, frame):
    frame.save(self.path + '/' + str(self.cur_frame_number) + '.png')
...
class Experiment:
  ...
  def load(self, claim=True):
    ...
    if claim:
      ...
      with tf.io.gfile.GFile(os.path.join(claim_dir, 'claim.txt'), 'wb') as f:
  ...
  def initialize_video_for_seed(self, seed, env_name, agent_id=None,
    ...
    vid_path = os.path.join(
        *[self.videos_dir, self.name, trim_env_name])
    ...
    vid_recorder = MyRecorder(vid_path, self.fps)
    return vid_recorder, matrix_path
  ...
  def run_seed_trials(self, seed, env_name, agent_id=0, num_trials=25,
    ...
    if self.save_matrices:
      with tf.io.gfile.GFile(matrix_filename, 'wb') as f:
  ...
  def create_env_with_adversary(self, policy, recorder):
    ...
    # print('cur save_matrice option:', self.save_matrices)
...
def load_existing_transfer_file(transfer_dir, transfer_csv, test_on_test=False,
  ...
  if transfer_csv:
    ...
    with tf.io.gfile.GFile(transfer_df_path, 'rb') as f:
      df = pd.read_csv(f)
...
def main(_):
  ...
  experiments =  Experiment('paired', 499950, num_agents=3,
                  benchmark_against=[499800, 499500])
      # Experiment('minimax', 17486367, num_agents=2),
      # Experiment('domain_randomization', 17486166, num_agents=1),
  experiments.policies['agent'] = tf.saved_model.load(os.path.join(FLAGS.root_dir, 'agent', '0', 'policy_000499500'))
  experiments.policies['adversary_env'] = tf.saved_model.load(os.path.join(FLAGS.root_dir, 'adversary_env', '0', 'policy_000499500'))

  experiments.save_matrices = True

  vid_path = os.path.join(FLAGS.root_dir, 'videos')

  recorder = MyRecorder(vid_path, 30)
  experiments.py_env, experiments.tf_env = experiments.load_environment(experiments.adv_env_name)

  experiments.blank_frame_encoding = experiments.py_env._gym_env.grid.encode()

  _, image = experiments.run_adversarial_trial(experiments.policies['adversary_env'], experiments.policies['agent'], None)

  for img_i in range(len(image)):
    img = Image.fromarray(image[img_i], 'RGB')  
    img.save('figs/envs/minimax' + str(img_i) + '.png')
